<x-app-layout>
    <x-slot name="header">
        <h2 class="h3 fw-bold m-0">⚽ Редактировать игрока</h2>
    </x-slot>
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card shadow-lg border-0">
                <div class="card-header bg-gradient-football text-white py-4">
                    <h5 class="m-0">
                        <i class="bi bi-pencil-square"></i> Редактирование: {{ $player->full_name }}
                    </h5>
                </div>
                    ...
                    <form action="{{ route('players.update', $player->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="full_name" id="fullName" class="form-control form-control-lg @error('full_name') is-invalid @enderror"
                                   value="{{ old('full_name', $player->full_name) }}" placeholder="Введите полное имя" required>
                            @error('full_name')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="position" id="position" class="form-control form-control-lg @error('position') is-invalid @enderror"
                                   value="{{ old('position', $player->position) }}" placeholder="Например: Вратарь, Защитник, Полузащитник, Нападающий" required>
                            @error('position')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
