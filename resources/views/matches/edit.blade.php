<x-app-layout>
    <x-slot name="header">
        <h2 class="h3 fw-bold m-0">⚽ Редактировать матч #{{ $match->id }}</h2>
    </x-slot>

    <div class="mb-4">
        <a href="{{ route('matches.show', $match->id) }}" class="btn btn-secondary">
            <i class="bi bi-arrow-left"></i> Вернуться к матчу
        </a>
    </div>
        ...
        <div class="col-md-8">
            <div class="card shadow-lg border-0">
                <div class="card-header bg-gradient-football text-white py-3">
                    <h5 class="m-0"><i class="bi bi-pencil"></i> Информация о матче</h5>
                </div>
                    ...
                    <form action="{{ route('matches.update', $match->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="date" id="date" class="form-control form-control-lg" 
                                   value="{{ $match->date ? $match->date->format('Y-m-d\TH:i') : '' }}" required>
                            @error('date')
                                <small class="text-danger d-block mt-1">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('matches.show', $match->id) }}" class="btn btn-outline-secondary btn-lg">
                                Отмена
                            </a>
                        </div>
                    </form>
